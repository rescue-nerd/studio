...
serve(async (req) => {
  ...
  try {
    ...
    const { data: associatedRecords, error: checkAssociationsError } = await supabaseClient
      .from('godowns')
      .select('id')
      .eq('branch_id', requestData.id)
      .limit(1)

    if (checkAssociationsError) {
      return new Response(
        JSON.stringify({
          success: false,
          error: {
            message: 'Error checking branch associations',
            details: checkAssociationsError.message
          }
        }),
        {
          status: 500,
      ...
      return new Response(
        JSON.stringify({
          ...
          error: {
            ...
            details: 'This branch has associated godowns. Please delete or reassign them first.'
          }
        }),
        {
          status: 400,
