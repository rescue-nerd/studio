...
serve(async (req) => {
  ...
  try {
      ...
      return new Response(
        JSON.stringify({
          ...
          error: {
            ...
            details: 'You must be logged in to delete a driver'
          }
        }),
        {
          status: 401,
    ...
    // Check if driver has any associated manifests
    const { data: associatedManifests, error: manifestError } = await supabaseClient
      .from('manifests')
      .select('id')
      .eq('driver_id', requestData.id)
      .limit(1)
    ...
    if (manifestError) {
      return new Response(
        JSON.stringify({
          success: false,
          error: {
            message: 'Error checking driver associations',
      ...
      return new Response(
        JSON.stringify({
          ...
          error: {
            ...
            details: 'This driver has associated manifests. Please delete or reassign them first.'
          }
        }),
        {
          status: 400,
